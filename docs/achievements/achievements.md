...
-   **How it was implemented:**
    1.  **`auto-fill` vs. `auto-fit`:** In `main-style.css`, the grid layout was changed from `grid-template-columns: repeat(auto-fit, ...)` to `repeat(auto-fill, ...)`. `auto-fill` creates empty tracks instead of stretching existing items, preserving their intended size.
