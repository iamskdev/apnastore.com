...
| `shared/styles/`                 | Global CSS files that apply to the whole app.                        | `main-style.css`, `theme.css`                   |
...
-   **Problem:** `shared/scripts/main-script.js` is the main entry point for `index.html`. Its primary job is to orchestrate the public-facing SPA. Placing it in `shared/` can be confusing, as `shared/` is meant for code that is *imported* by other scripts, not for a main entry point itself.
