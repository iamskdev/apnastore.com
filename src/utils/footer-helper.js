...
export function initializeFooter(containerElement, role) {
    ...
    if (!footerWrapper.dataset.scrollListenerAttached) {
        ...
        const handleScrollCollapse = () => {
            ...
            // Only run this check if the footer's view is active and the footer is expanded.
            if (!containerElement.classList.contains('view-active') || !footerWrapper.classList.contains('is-expanded')) {
                lastScrollTop = currentScrollTop; // Keep track of position even when collapsed
                return;
            }

            // This is the key fix: only proceed if the user is scrolling UP.
            // A scroll down (currentScrollTop > lastScrollTop) should never collapse the footer.
            if (currentScrollTop > lastScrollTop) {
                lastScrollTop = currentScrollTop; // Update position and exit
                return;
            }

            // If we reach here, the user is scrolling UP.
            const footerRect = footerWrapper.getBoundingClientRect();
            const tabNav = document.getElementById('bottom-nav-placeholder'); // Assuming this is your tab navigation bar
            const tabNavHeight = tabNav ? tabNav.offsetHeight : 0;

            // This logic collapses the footer if its bottom edge scrolls up above the top edge of the tab navigation bar.
            // This check is now only evaluated on an upward scroll.
            if (footerRect.bottom < (window.innerHeight - tabNavHeight - 10)) { // Added a small buffer (10px)
                collapseFooter();
            }

            lastScrollTop = currentScrollTop; // Update position for the next event
        };

        containerElement.addEventListener('scroll', handleScrollCollapse, { passive: true });
