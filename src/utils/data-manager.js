...
const createDataFetcher = (collectionName, idKey) => {
    ...
    const fetchAll = (force = false) => {
        ...
        const promise = (async () => {
            if (dataSource === 'localstore') {
                ...
                try {
                    const snapshot = await window._firestore.collection(collectionName).get();
                    // Map documents to include their ID, making it consistent with fetchById.
                    const data = snapshot.docs.map(doc => ({
                        [idKey]: doc.id, ...doc.data()
                    }));
    ...
    const fetchById = async (id) => {
        ...
        if (dataSource === 'localstore') {
            ...
            try {
                const doc = await window._firestore.collection(collectionName).doc(id).get();
                if (doc.exists) {
                    // Return data consistent with schema, including the ID from the doc itself.
                    return { [idKey]: doc.id, ...doc.data() };
                } else {
...
export async function generateSequentialId(collectionName, prefix) {
    ...
    const counterRef = window._firestore.collection('counters').doc(collectionName);

    try {
        let newIdNumber;
        await window._firestore.runTransaction(async (transaction) => {
...
export async function fetchActivePromotion() {
    ...
    if (dataSource === 'localstore') {
        ...
        try {
            const snapshot = await window._firestore.collection('promotions').where('meta.status.isActive', '==', true).limit(1).get();
            if (snapshot.empty) {
                return null;
            }
