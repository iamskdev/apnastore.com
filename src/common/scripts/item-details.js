...
import { initializeDesktopHoverZoom } from '/src/utils/cursor-zoom.js';
 ...
 * This function is exported to be used by main-script.js.
...
export async function displayRelatedItems(currentItem) {
  ...
  try {
    ...
    if (!items.length) {
      const res = await fetch('/firebase/data/items.json');
...
export function initializeItemDetailsPage() {
  ...
  try {
    ...
    const initZoom = async () => {
      const { initializeImageZoom } = await import('/src/utils/image-zoom.js');
    ...
    feedbackBtn.addEventListener('click', async () => {
      ...
      if (!isModalInitialized) {
        try {
          ...
          const response = await fetch('/src/components/feedback-modal/feedback-modal.html');
          if (!response.ok) throw new Error(`Failed to load modal HTML: ${response.status}`);
          modalContainer.innerHTML = await response.text();

          // 2. Import the modal's JavaScript module
          const modalModule = await import('/src/components/feedback-modal/feedback-modal.js');
