...
function setButtonLoading(button, isLoading) {
    ...
    if (isLoading) {
        ...
        // मूल HTML को पुनर्स्थापित करें, यदि यह संग्रहीत है
...
function setupPhoneVerificationUI() {
    ...
    otpBtn.addEventListener('click', async () => {
        ...
        const success = await AuthService.sendVerificationOtp(phoneInput.value);
    ...
    otpVerifyBtn.addEventListener('click', async () => {
        const success = await AuthService.verifyPhoneOtp(otpInput.value);
        if (success) {
 ...
 * view-manager.js द्वारा कॉल किया गया।
 */
export function init() {
    ...
    registerForm?.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!AuthService.validateForm(registerForm)) return;
    ...
    // सोशल लॉगिन बटन
    document.querySelector('.social-btn.google')?.addEventListener('click', AuthService.signInWithGoogle);
    document.querySelector('.social-btn.apple')?.addEventListener('click', AuthService.signInWithApple);
}
